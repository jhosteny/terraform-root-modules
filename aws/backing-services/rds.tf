...
locals {
  ...
  rds_admin_password = length(var.rds_admin_password) > 0 ? var.rds_admin_password : join("", random_password.rds_admin_password.*.result)
  rds_db_name        = join("", random_pet.rds_db_name.*.id)
}

module "rds" {
  source                      = "git::https://github.com/cloudposse/terraform-aws-rds.git?ref=tags/0.16.0"
...
resource "aws_ssm_parameter" "rds_engine_version" {
  count       = local.rds_enabled ? 1 : 0
  name        = format(var.chamber_parameter_name, local.chamber_service, "rds_engine_version")
...
resource "aws_ssm_parameter" "rds_security_group_id" {
  count       = local.rds_enabled ? 1 : 0
  name        = format(var.chamber_parameter_name, local.chamber_service, "rds_security_group_id")
